...
class GroupedQueryAttention(nn.Module):
    """
    GQA: query heads are divided into n_kv_heads groups; each group shares
    one K head and one V head.

    Special cases:
      n_kv_heads == n_heads  →  standard MHA
      n_kv_heads == 1        →  MQA

    Set cfg.n_kv_heads in config.py (default 2).
    Reference: Ainslie et al. 2023 – "GQA: Training Generalised Multi-Query..."
    """

    def __init__(self, d_model: int, h: int, dropout: float, cfg=None) -> None:
        super().__init__()
        assert d_model % h == 0, "d_model must be divisible by h"
        n_kv = cfg.n_kv_heads if cfg is not None else max(1, h // 4)
        assert h % n_kv == 0, \
            f"n_heads ({h}) must be divisible by n_kv_heads ({n_kv})"

        self.d_model    = d_model
        self.h          = h
        self.n_kv_heads = n_kv
        self.n_rep      = h // n_kv       # query heads per KV head
        self.d_k        = d_model // h

        self.w_q     = nn.Linear(d_model, h * self.d_k,   bias=False)
        self.w_k     = nn.Linear(d_model, n_kv * self.d_k, bias=False)
        self.w_v     = nn.Linear(d_model, n_kv * self.d_k, bias=False)
        self.w_o     = nn.Linear(d_model, d_model,          bias=False)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, q, k, v, mask=None):
        ...
        Q = self.w_q(q).view(B, T, self.h,          self.d_k).transpose(1, 2)
        K = self.w_k(k).view(B, T, self.n_kv_heads, self.d_k).transpose(1, 2)
        V = self.w_v(v).view(B, T, self.n_kv_heads, self.d_k).transpose(1, 2)

        # Tile each KV head to cover its n_rep query heads
        K = K.repeat_interleave(self.n_rep, dim=1)   # (B, h, T, d_k)
